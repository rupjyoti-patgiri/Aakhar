...
// // --- API Functions ---
// const fetchPost = (id) => apiClient.get(`/posts/getPost/${id}`).then(res => res.data.data);
// const toggleLike = ({ postId, user, hasLiked, likeId }) => hasLiked ? apiClient.delete(`/likes/deleteLike/${postId}/${likeId}`) : apiClient.post('/likes/createLike', { postId, user: user.username });
// const createComment = ({ postId, body, user }) => apiClient.post('/comments/createComment', { postId, body, user: user.username });
// const updatePost = ({ postId, title, body }) => apiClient.put(`/posts/updatePost/${postId}`, { title, body });
// const deletePost = (postId) => apiClient.delete(`/posts/deletePost/${postId}`);
...
//             onSuccess: (response) => {
//                 toast.success(successMessage);
//                 // **THE FIX**: Instead of just invalidating, we immediately update the cache
//                 // with the fresh, complete data returned from the fixed backend. This prevents crashes.
//                 queryClient.setQueryData(['post', postId], response.data.data);
//                 queryClient.invalidateQueries({ queryKey: ['posts'] }); // Invalidate the list view
//             },
//             onError: (err) => toast.error(err.response?.data?.message || "An error occurred."),
//         });
//     };

//     const likeMutation = handleMutation(toggleLike, { successMessage: "Success!" });
//     const commentMutation = handleMutation(createComment, { successMessage: "Comment posted!" });
//     const updatePostMutation = handleMutation(updatePost, { successMessage: "Post updated!" });
//     const deletePostMutation = useMutation({
//         mutationFn: deletePost,
//         onSuccess: () => {
//             toast.success("Post deleted!");
...
//     const onToggleLike = () => user && likeMutation.mutate({ postId, user, hasLiked: !!currentUserLike, likeId: currentUserLike?._id });
//     const onCommentSubmit = (data) => user && commentMutation.mutate({ postId, body: data.commentBody, user }, { onSuccess: () => commentForm.reset() });
//     const onEditSubmit = (data) => updatePostMutation.mutate({ postId, ...data }, { onSuccess: () => setIsEditing(false) });
//     const onDeleteConfirm = () => deletePostMutation.mutate(postId);

//     const isPostOwner = post?.author?._id === user?.id;
//     const canManagePost = isAdmin || isPostOwner;

//     // --- Render Logic ---
//     if (isLoading) return <div className="max-w-4xl mx-auto space-y-4"><Skeleton className="h-24 w-full" /><Skeleton className="h-96 w-full" /></div>;
//     if (isError || !post) return <div className="text-center py-10"><h1>404</h1><p>Post not found.</p></div>;
...
//                                     <DialogDescription>This will permanently delete this post and all its comments.</DialogDescription>
//                                     <DialogFooter>
//                                         <DialogClose asChild><Button type="button" variant="ghost">Cancel</Button></DialogClose>
//                                         <Button type="button" variant="destructive" onClick={onDeleteConfirm} disabled={deletePostMutation.isLoading}>Delete Post</Button>
//                                     </DialogFooter>
//                                 </DialogContent>
//                             </Dialog>
//                         </div>
...
//                         <Avatar className="h-10 w-10"><AvatarImage src={author.avatar?.imageUrl} /><AvatarFallback>{author.username.charAt(0).toUpperCase()}</AvatarFallback></Avatar>
//                         <div><p className="font-semibold text-foreground">{author.username}</p><p className="text-sm">Published on {new Date(post.createdAt).toLocaleDateString()}</p></div>
//                     </div>
...
//                 <div className="flex items-center gap-4 border-t border-b py-4 mb-8">
//                     <Button variant={currentUserLike ? "default" : "outline"} onClick={onToggleLike} disabled={!user || likeMutation.isLoading}><ThumbsUp className="mr-2 h-4 w-4" />{currentUserLike ? 'Liked' : 'Like'} ({post.likes.length})</Button>
//                     <div className="flex items-center gap-2 text-muted-foreground"><MessageCircle className="h-5 w-5" /><span>{post.comments?.length || 0} Comments</span></div>
//                 </div>

//                 <section className="mt-12">
//                     <h2 className="text-2xl font-bold mb-6">Comments</h2>
//                     {user ? (
//                         <form onSubmit={commentForm.handleSubmit(onCommentSubmit)} className="mb-8"><Textarea placeholder="Write a comment..." {...commentForm.register("commentBody", { required: true })} /><Button type="submit" className="mt-2" disabled={commentMutation.isLoading}>Post Comment</Button></form>
//                     ) : <p className="text-muted-foreground mb-8"><Link to="/login" className="underline font-semibold">Log in</Link> to post a comment.</p>}
                    
//                     <div className="space-y-6">
//                         {post.comments?.slice().reverse().map(comment => <Comment key={comment._id} comment={comment} postId={postId} postAuthorId={author?._id} />)}
//                     </div>
//                 </section>
//             </article>
...
//                         <div><label htmlFor="title" className="font-medium text-sm">Title</label><Input id="title" {...editForm.register("title", { required: true })} /></div>
//                         <div><label htmlFor="body" className="font-medium text-sm">Content</label><Textarea id="body" rows={12} {...editForm.register("body", { required: true })} /></div>
//                         <DialogFooter><Button type="button" variant="ghost" onClick={() => setIsEditing(false)}>Cancel</Button><Button type="submit" disabled={updatePostMutation.isLoading}>{updatePostMutation.isLoading ? "Saving..." : "Save Changes"}</Button></DialogFooter>
//                     </form>
//                 </DialogContent>
//             </Dialog>
//         </>
